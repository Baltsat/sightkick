...
if [[ "$build_version" != '1.2.2' ]]; then
    echo "Expected CFBundleVersion 1.2.2, got $build_version" >&2
    exit 1
fi
...
assert(integrity.application.releaseVersion === '1.2.0-kb.2', 'Wrong release version');
assert(integrity.application.shortVersion === '1.2.0', 'Wrong short version');
assert(integrity.application.buildVersion === '1.2.2', 'Wrong build version');
