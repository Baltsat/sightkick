...
if [[ "$target_path" == *.dmg ]]; then
    ...
    if [[ "$(basename "$dmg_path")" != 'Drumroll-1.2.0-kb.10-arm64.dmg' ]]; then
...
if [[ "$build_version" != '1.2.10' ]]; then
    echo "Expected CFBundleVersion 1.2.10, got $build_version" >&2
    exit 1
fi
...
assert(integrity.application.releaseVersion === '1.2.0-kb.10', 'Wrong release version');
assert(integrity.application.shortVersion === '1.2.0', 'Wrong short version');
assert(integrity.application.buildVersion === '1.2.10', 'Wrong build version');
