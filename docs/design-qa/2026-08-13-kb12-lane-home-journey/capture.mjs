...
// Integration-phase capture for the kb.12 five-lane merge. Modelled on
// docs/design-qa/2026-08-12-practice-visual/capture-practice-visual.mjs
// (same production Electron build + final-QA fixture + keyboard input
// device), updated for the unified library UI (SongSearch/song-search
// testid, no more Local/Drums/Favorites tabs).
...
async function _captureLibrary(page) {
...
async function captureJourney(page) {
  ...
  // Proof for the two interactive fixes: plaques reveal on hover instead of
  // floating permanently, and the "Controls" toggle now has real chrome and
  // genuinely opens/closes.
  await page.setViewportSize(wideViewport);

  const lockedNode = page
    .locator(
      '[data-testid^="lesson-item-"][data-in-journey-viewport="true"][data-locked="true"]',
    )
    .first();

  if (await lockedNode.count()) {
    await lockedNode.hover();
    await page.waitForTimeout(200);
    await page.screenshot({
      animations: 'disabled',
      path: path.join(outputDir, '03b-journey-node-hover-1225x768.png'),
    });
  }

  await page.mouse.move(20, 20);

  const controlsToggle = page.getByTestId('journey-controls-toggle').first();

  await controlsToggle.click();
  await page.waitForTimeout(200);
  await page.screenshot({
    animations: 'disabled',
    path: path.join(outputDir, '03c-journey-controls-open-1225x768.png'),
  });
  await controlsToggle.click();
}
...
async function _capturePractice(page) {
  await openPracticeReady(page);
  await shootBoth(page, '04-practice');
}

async function _captureResult(page) {
...
async function run() {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'drumroll-finish-visual-'),
  );
  const pageErrors = [];
  let failure;
  ...
  try {
    ...
    // Home/LessonsJourney/LessonsView/StreakMeter lane only - library,
    // practice, and result are mid-edit by other lanes on this checkout.
