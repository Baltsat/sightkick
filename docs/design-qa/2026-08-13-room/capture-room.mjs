...
// Integrator-phase capture after the kb.13 home-composition + journey/practice-sizing
// lanes merged. Modelled on docs/design-qa/2026-08-13-truth/capture-truth.mjs.
...
async function run() {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'drumroll-room-visual-'),
  );
  const pageErrors = [];
  let failure;
