...
async function assert_journey_world_title_fits(page) {
  for (const viewport of [wide_viewport, compact_viewport]) {
...
async function capture_journey(page) {
  ...
  await assert_journey_world_title_fits(page);
...
async function run() {
  ...
  const user_data_dir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'drumroll-kb14-installed-'),
  );
  const page_errors = [];
  let failure;
