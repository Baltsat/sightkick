...
// Integration-close capture for the 2026-08-13 four-lane merge (icon,
// learner-owned tempo / hands-free start, explanations that teach, and the
// read-only verification pass). Modelled on
// docs/design-qa/2026-08-13-finish/capture-finish.mjs — same production
// Electron build (out/main/index.js, so it reflects the current working
// tree including every lane's uncommitted edits) + final-QA fixture +
...
async function run() {
  ...
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'drumroll-final-visual-'),
  );
  const pageErrors = [];
  let failure;
